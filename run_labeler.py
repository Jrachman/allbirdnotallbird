...
def change_img_index(x):
    ...
    if WITH_QT:
        ...
        print(f"Showing image {img_index}/{last_img_index} path:{img_path}")
...
def change_class_index(x):
    ...
    class_index = x
    if WITH_QT:
        cv2.displayOverlay(WINDOW_NAME, f"Selected class {class_index}/{last_class_index}\n{class_list[class_index]}", 3000)
    else:
...
def yolo_format(class_index, point_1, point_2, width, height):
    ...
    return f"{class_index} {x_center} {y_center} {x_width} {y_height}"
...
while True:
    ...
    if point_1[0] is not -1:
        ...
        if point_2[0] is not -1:
            ...
            if WITH_QT:
                cv2.displayOverlay(WINDOW_NAME, f"Selected label: {class_list[class_index]}\nPress [w] or [s] to change.", 120)
    ...
    # help key listener
        ...
        if WITH_QT:
            cv2.displayOverlay(WINDOW_NAME, f"[e] to show edges;\n[q] to quit;\n[a] or [d] to change Image;\n[w] or [s] to change Class.\n{img_path}", 6000)
        else:
            print(f"[e] to show edges;\n[q] to quit;\n[a] or [d] to change Image;\n[w] or [s] to change Class.\n{img_path}")
